...
namespace IKEA.BLL.Services.DepartmentServices
{
    ...
    {   //CLR Make all these things , i donot create the obj by myself , this done by CLR , and i determine its lifecycle ,,,,, make one obj per request (one obj for repository , one for services , one for options , ..
        ...
        public async Task <IEnumerable<DepartmentDto>> GetAllDepartments()
        {
            ...
            //Manual Mapping 
            var Departments =await unitOfWork.DepartmentRepository.GetAll().Where(D=>!D.IsDeletd).Select(dept => new DepartmentDto()
            {
                Id = dept.Id,
                Name = dept.Name,
                Code = dept.Code,
        ...
        public async Task<DepartmentDetailsDto>? GetDepartmentById(int id)         //make it nullable as if it is null not get error 
         {
        ...
        public async Task<int> CreateDepartment(CreatedDepartmentDto departmentDto)   //here we enter CreatedDepartmentDto as Department
        {
